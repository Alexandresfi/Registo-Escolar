...
function AddRelatorio() {
 //adicionando os números e os alunos no relatório 
let tabelaRela = document.getElementsByClassName("aluno")[0];
let tr = document.createElement("tr");
    let td1 = document.createElement("td");
    let td2 = document.createElement("td");
    let td3 = document.createElement("td");
    ...
    //colocando os inpuit
    td3.innerHTML = `<input type= number name=notas class='media1 T ${pos}'  placeholder=nota>`
    td4.innerHTML = `<input type= number name=notas class='media2 T ${pos}'  placeholder=nota>`
    td5.innerHTML = `<input type= number name=notas class='media3 T ${pos}'  placeholder=nota>`
    td7.innerHTML = `<input type= number name=notas class='media3 T ${pos}'  placeholder=nota>`
    td8.innerHTML = `<input type= number name=notas class='media3 T ${pos}'  placeholder=nota>`
    //calculando médias do Trimestre.
    document.getElementById("media").addEventListener("click",()=>{
        let nota1=parseInt(document.getElementsByClassName("nota "+ pos)[0].value);
        let nota2=parseInt(document.getElementsByClassName("nota "+ pos)[1].value);
...
document.getElementById("levaN").addEventListener("click",()=> {
    if(alunos==0){
...
document.getElementById("levaR").addEventListener("click",()=>{
    if(alunos==0){
        alert("Por favor, cadestre pelo menos um aluno!")
    }
    document.getElementById("coisa1").style.display = "none";
...
document.getElementById("ulti").addEventListener("click",()=>{
    document.getElementById("rela").style.display = "none";
    document.getElementById("coisa1").style.display = "block";  
})
document.getElementsByClassName("gerar")[0].addEventListener("click",()=> alert("Estamos trabalhando nesta funcionalidade"))
